...
- **`glutIdleFunc(update)`**: Đăng ký hàm `update` để được gọi liên tục khi chương trình "rảnh rỗi". Đây là cách hiệu quả để tạo animation liên tục, nhưng có thể chiếm dụng nhiều CPU.
...
- **Đa thức Bernstein (`B_k,n(t)`)**: Đây là các hàm cơ sở (basis functions) của đường cong/mặt cong Bezier. Chúng quyết định "trọng số" hay "mức độ ảnh hưởng" của mỗi điểm điều khiển lên một điểm cụ thể trên bề mặt.
    ...
    - `B_2,3(t) = 3t^2(1-t)`
    - `B_3,3(t) = t^3`
- **Tạo Lưới Đa Giác (Polygon Mesh)**: Bằng cách cho tham số `u` và `v` chạy từ 0 đến 1 theo các bước nhỏ (ví dụ: `1.0 / DIVISIONS`), chúng ta tính toán ra một lưới các điểm trên bề mặt. Các điểm này sau đó được nối lại với nhau để tạo thành các dải tam giác (`GL_TRIANGLE_STRIP`), hình thành nên mô hình 3D của mặt sóng.
...
- **Thực hiện**: Trong hàm `update`, chúng ta thay đổi tọa độ `y` của các điểm điều khiển `controlPoints[i][j]` bằng các hàm lượng giác như `sin()` và `cos()`. Vì các điểm điều khiển thay đổi, bề mặt Bezier được tính toán lại trong mỗi frame, tạo ra chuyển động.
...
- **Ứng dụng**: Khi vẽ nhiều vật thể, chúng ta dùng cặp lệnh này để đảm bảo các phép biến đổi của vật thể này không ảnh hưởng đến các vật thể khác.

#### **3.2. Các Phép Biến Đổi Cơ Bản**

- **`glTranslatef(x, y, z)`**: Tịnh tiến (di chuyển) hệ tọa độ hiện tại.
- **`glRotatef(angle, x, y, z)`**: Quay hệ tọa độ hiện tại một góc `angle` quanh trục vector `(x, y, z)`. Đây là cách thực hiện "phép quay quanh một trục bất kỳ" trong OpenGL.
- **`glScalef(x, y, z)`**: Co giãn hệ tọa độ.

#### **3.3. Camera và Phép Chiếu**

- **Mô hình Camera (`gluLookAt`)**: Hàm này định nghĩa phép biến đổi quan sát (Viewing Transformation), chuyển từ hệ tọa độ thế giới (WCS) sang hệ tọa độ quan sát (VCS). Nó nhận 3 tham số:
...
- **Phép Chiếu Phối Cảnh (`gluPerspective`)**: Hàm này định nghĩa phép chiếu phối cảnh (Perspective Projection), tạo ra cảm giác về chiều sâu (vật ở xa trông nhỏ hơn). Nó nhận các tham số:
    ...
    3.  `zNear`, `zFar`: Khoảng cách đến mặt phẳng cắt gần và mặt phẳng cắt xa.
...
- **Nguồn sáng (`glLightfv`)**:
    - `GL_POSITION`: Vị trí của nguồn sáng.
...
- **Vật liệu (`glMaterialfv`)**:
    ...
    - `GL_SHININESS`: Độ bóng của vật liệu, ảnh hưởng đến kích thước và độ sắc nét của đốm sáng specular. Giá trị càng cao, đốm sáng càng nhỏ và sắc nét.

#### **4.3. Vector Pháp Tuyến (Normal Vector) - Chìa khóa của Chiếu sáng**
Vector pháp tuyến là một vector đơn vị, vuông góc với bề mặt tại một điểm. Nó quyết định bề mặt đó đang "hướng" về đâu, và là thông tin quan trọng nhất để các mô hình chiếu sáng (như Phong) tính toán màu sắc.

#### **4.4. Phương pháp tính Vector Pháp tuyến cho Mặt cong Bezier (Nâng cao)**
Để có hiệu ứng lấp lánh chân thực, vector pháp tuyến phải được tính toán chính xác tại *mỗi đỉnh* và thay đổi theo sự uốn lượn của sóng.

*   **Lý thuyết:** Đối với một bề mặt tham số như mặt Bezier `P(u,v)`, vector pháp tuyến `N` tại một điểm có thể được tìm thấy bằng cách lấy **tích có hướng (cross product)** của hai vector tiếp tuyến (tangent vectors) tại điểm đó. Hai vector tiếp tuyến này chính là các **vector đạo hàm riêng (partial derivative vectors)** của `P` theo `u` và `v`.
    *   `N(u,v) = (∂P/∂u) × (∂P/∂v)`
*   **Đạo hàm của Đa thức Bernstein:** Để tính `∂P/∂u` và `∂P/∂v`, chúng ta cần tính đạo hàm của các đa thức Bernstein. Công thức đạo hàm là:
    *   `B'_i,n(t) = n * (B_{i-1,n-1}(t) - B_i,n-1(t))`
*   **Triển khai:**
    1.  Trong mỗi frame, tại mỗi đỉnh của lưới (tương ứng với một cặp `u,v`), chúng ta tính hai vector `dU = ∂P/∂u` và `dV = ∂P/∂v`.
    2.  Tính vector pháp tuyến `N = dU × dV`.
    3.  **Chuẩn hóa (Normalize)** vector `N` để nó trở thành vector đơn vị (có độ dài bằng 1).
    4.  Cung cấp vector pháp tuyến đã chuẩn hóa này cho OpenGL bằng lệnh `glNormal3f()` trước khi vẽ đỉnh tương ứng.
*   **Ưu điểm:** Phương pháp này cho kết quả chính xác về mặt toán học, tạo ra các pháp tuyến thay đổi mượt mà trên bề mặt, giúp các đốm sáng specular di chuyển một cách tự nhiên trên các đỉnh sóng. Nó vượt trội hơn so với phương pháp "tính trung bình pháp tuyến các mặt" trước đây.

#### **4.5. Tô Bóng (Shading Models)**
...
### **Chương 5: Kỹ Thuật Mở Rộng (Đã loại bỏ trong phiên bản hiện tại)**

*Ghi chú: Các kỹ thuật này đã được triển khai ở các phiên bản trước nhưng đã được loại bỏ để tập trung vào chất lượng bề mặt sóng.*
